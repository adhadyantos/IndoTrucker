    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                final StorageReference profileFilepath = mImageStorage.child("profile_images").child(current_user_id+".jpg");
                final StorageReference thumbFilepath = mImageStorage.child("profile_images").child("thumbs").child(current_user_id + ".jpg");


                profileFilepath.putFile(resultUri).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                    @Override
                    public Task<Uri> then(@NonNull Task<UploadTask.TaskSnapshot> task) throws Exception {
                        if (!task.isSuccessful()) {
                            throw task.getException();
                        }
                        return profileFilepath.getDownloadUrl();
                    }
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        if (task.isSuccessful()) {
                            ...
                            String profileUrl = imageUri.toString();
                            mUserDatabase.child("image").setValue(profileUrl);
                            ...
                            mProgressDialog.dismiss();
                            Toast.makeText(SettingsActivity.this, "Failed to Upload the Image", Toast.LENGTH_LONG).show();
                        }
                    }
                });

                thumbFilepath.putBytes(thumb_byte).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                    @Override
                    ...
                    public void onComplete(@NonNull Task<Uri> task) {
                        if (task.isSuccessful()) {
                            ...
                            String thumbnailUrl = thumbUri.toString();
                            mUserDatabase.child("thumb_image").setValue(thumbnailUrl);

                            mProgressDialog.dismiss();
                            Toast.makeText(SettingsActivity.this, "Thumbnail Uploaded", Toast.LENGTH_LONG).show();

                        } else {
                            mProgressDialog.dismiss();
                            Toast.makeText(SettingsActivity.this, "Failed to Upload the Thumbnail", Toast.LENGTH_LONG).show();
                        }
                    }
                });
                /*
                profileFilepath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        mUserDatabase.child("image").setValue(imageUrl);
                    }
                });
                ...
                thumbFilepath.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        mUserDatabase.child("thumb_image").setValue(thumbUrl);
                    }
                });
                */
